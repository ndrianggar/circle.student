...
<script type="text/javascript" src="jquery-1.4.4.min.js"></script>
  ...
  <body onunload=keluar() class="hold-transition skin-blue layout-top-nav" id="main">
    <div class="wrapper">
      <div id="mySidenav" class="sidenav">
  <a href="javascript:void(0)" class="closebtn" onclick="closeNav()">&times;</a>
  <?php 
  $pg = mysql_query("SELECT a.id_jadwal, b.id_cbt, d.* FROM `rb_quiz_jadwal` a JOIN rb_quiz_cbt b ON a.id_cbt=b.id_cbt JOIN rb_quiz_cbt_soal c ON b.id_cbt=c.id_cbt
                          JOIN rb_bank_pertanyaan_objektif d ON c.id_bank_pertanyaan_objektif=d.id_bank_pertanyaan_objektif where a.id_jadwal='$_GET[id_jadwal]' AND kategori_soal = 'objektif'");
  $essay = mysql_query("SELECT a.id_jadwal, b.id_cbt, d.* FROM `rb_quiz_jadwal` a JOIN rb_quiz_cbt b ON a.id_cbt=b.id_cbt JOIN rb_quiz_cbt_soal c ON b.id_cbt=c.id_cbt
                          JOIN rb_bank_pertanyaan_objektif d ON c.id_bank_pertanyaan_objektif=d.id_bank_pertanyaan_objektif where a.id_jadwal='$_GET[id_jadwal]' AND kategori_soal = 'essay'");
  $i = 1;
  if (mysql_num_rows($pg)>0) {
    echo "<h3>Pilihan Ganda</h3>";
  }
  while($rows = mysql_fetch_array($pg)){ 
    $halaman = $i-1;
        echo "<a onclick=\"page($halaman)\" class='step btn btn-primary' style='width:40px; margin:5px;'>$i</a>";
    $i++;
  } 
  if (mysql_num_rows($essay)) {
    echo "<h3>Essay</h3>";
  }
  while($rows = mysql_fetch_array($essay)){ 
    $halaman = $i-1;
      echo "<a onclick=\"page($halaman)\" class='step btn btn-primary' style='width:40px; margin:5px;'>$i</a>";
    $i++;
  } 
  ?>
      ...
      <div class="content-wrapper">
        ...
        <div class='col-xs-12'> 
        <a href="#" class="btn btn-primary btn-flat" style="font-size:30px;cursor:pointer" id="btnNo" onclick="openNav()"> <i class="fa fa-angle-double-right"> </i></a> 
          <div class='box'>
                ...
                <div class='col-sm-12' style='min-height:400px'>  
                    ...
                    while($rows = mysql_fetch_array($paging)){ 
                        ...
                        if ($rows['kategori_soal']=='objektif'){
                            echo "<a onclick=\"page($halaman)\" class='step btn btn-default' style='display:none;'>$i</a>";
                        }else{
                            echo "<a onclick=\"page($halaman)\" class='step btn btn-default' style='display:none; color:red'>$i</a>";
                        }
                        $i++;
                    } 
                    ?>
                    ...
                    <form action="quiz_siswa_mulai.php?act=hasil&id_jadwal=<?php echo $_GET['id_jadwal']; ?>" method="post" id="formulir">
                        ...
                        $jumlah = mysql_query("SELECT a.id_jadwal, b.id_cbt, d.* FROM `rb_quiz_jadwal` a JOIN rb_quiz_cbt b ON a.id_cbt=b.id_cbt JOIN rb_quiz_cbt_soal c ON b.id_cbt=c.id_cbt
                                            JOIN rb_bank_pertanyaan_objektif d ON c.id_bank_pertanyaan_objektif=d.id_bank_pertanyaan_objektif where a.id_jadwal='$_GET[id_jadwal]' ORDER BY d.kategori_soal asc, RAND()");
                        ...
                        while($row = mysql_fetch_array($jumlah)){
                            echo "<div class='tab'>
                                        ...
                                        <input type='hidden' value='$row[id_bank_pertanyaan_objektif]' name='id$no' id='pertanyaan'>";
                                        $checkJwb = mysql_query("SELECT * FROM rb_jawaban_objektif WHERE id_jadwal = '$_GET[id_jadwal]' AND id_pertanyaan_objektif = '$row[id_bank_pertanyaan_objektif]' AND id_siswa = '$_SESSION[ids]'");
                                        if ($row['kategori_soal']=='objektif'){
                                          if (mysql_num_rows($checkJwb)>0) {
                                            while($jwb = mysql_fetch_array($checkJwb)){ 
                                            echo "<p><span style='float:left; margin:-2px 5px 0px 0px'><input type='radio' name='jawaban$no' value='A' onclick='validateForm(),alerts($(this).val(),$row[id_bank_pertanyaan_objektif])'"; if (A==$jwb[jawaban]) {
                                              echo "checked='checked'";
                                            } echo"> A. </span>$row[jawab_a]</p>
                                                    <p><span style='float:left; margin:-2px 5px 0px 0px'><input type='radio' name='jawaban$no' value='B' onclick='validateForm(),alerts($(this).val(),$row[id_bank_pertanyaan_objektif])'"; if (B==$jwb[jawaban]) {
                                              echo "checked='checked'";
                                            } echo"> B. </span>$row[jawab_b]</p>
                                                    <p><span style='float:left; margin:-2px 5px 0px 0px'><input type='radio' name='jawaban$no' value='C' onclick='validateForm(),alerts($(this).val(),$row[id_bank_pertanyaan_objektif])'"; if (C==$jwb[jawaban]) {
                                              echo "checked='checked'";
                                            } echo"> C. </span>$row[jawab_c]</p>
                                                    <p><span style='float:left; margin:-2px 5px 0px 0px'><input type='radio' name='jawaban$no' value='D' onclick='validateForm(),alerts($(this).val(),$row[id_bank_pertanyaan_objektif])'"; if (D==$jwb[jawaban]) {
                                              echo "checked='checked'";
                                            } echo"> D. </span>$row[jawab_d]</p>
                                                    <p><span style='float:left; margin:-2px 5px 0px 0px'><input type='radio' name='jawaban$no' value='E' onclick='validateForm(),alerts($(this).val(),$row[id_bank_pertanyaan_objektif])'"; if (E==$jwb[jawaban]) {
                                              echo "checked='checked'";
                                            } echo"> E. </span>$row[jawab_e]</p>";
                                            }
                                          }else{
                                            echo "<p><span style='float:left; margin:-2px 5px 0px 0px'><input type='radio' name='jawaban$no' value='A' onclick='validateForm(),alerts($(this).val(),$row[id_bank_pertanyaan_objektif])'> A. </span>$row[jawab_a]</p>
                                                    <p><span style='float:left; margin:-2px 5px 0px 0px'><input type='radio' name='jawaban$no' value='B' onclick='validateForm(),alerts($(this).val(),$row[id_bank_pertanyaan_objektif])'> B. </span>$row[jawab_b]</p>
                                                    <p><span style='float:left; margin:-2px 5px 0px 0px'><input type='radio' name='jawaban$no' value='C' onclick='validateForm(),alerts($(this).val(),$row[id_bank_pertanyaan_objektif])'> C. </span>$row[jawab_c]</p>
                                                    <p><span style='float:left; margin:-2px 5px 0px 0px'><input type='radio' name='jawaban$no' value='D' onclick='validateForm(),alerts($(this).val(),$row[id_bank_pertanyaan_objektif])'> D. </span>$row[jawab_d]</p>
                                                    <p><span style='float:left; margin:-2px 5px 0px 0px'><input type='radio' name='jawaban$no' value='E' onclick='validateForm(),alerts($(this).val(),$row[id_bank_pertanyaan_objektif])'> E. </span>$row[jawab_e]</p>";
                                          }
                                        }else{
                        ...
                        <div style="overflow:auto;">
                              <button class='btn btn-sm btn-warning pull-left' type="button" id="prevBtn" onclick="nextPrev(-1)"><i class="fa fa-angle-left"></i> Kembali</button>
                              <button class='btn btn-sm btn-success pull-right' type="button" id="nextBtn" onclick="nextPrev(1)">Selanjutnya <i class="fa fa-angle-right"></i></button>
                              <button class='btn btn-sm btn-danger pull-right' type="button" id="finishBtn" onclick="$('#confirmModal').modal('show')" style="display: none;">Finish <i class="fa fa-angle-right"></i></button>
                        </div><br>
                    </form>
                ...
                echo "<div class='col-md-12' style='margin-bottom:10%'>";
                    ...
                    for ($i=0; $i<=$n; $i++){
                        ...
                        if ($jenis['kategori_soal']=='objektif'){
                            ...
                            mysql_query("UPDATE `rb_jawaban_objektif` SET `status` = 'FINISHED' WHERE id_jadwal = '$_GET[id_jadwal]' AND id_siswa = '$_SESSION[ids]'");
                        }else{
                    ...
                    if ($essay>=1){
                      if ($t[jenis_cbt]=="Try Out"|| $t[jenis_cbt]=="Penilaian Akhir Tahun") {
                         echo "<center><h3><span style='color:green'>Terima Kasih Anda Telah Menyelesaikan Soal $t[jenis_cbt].</h3>";
...
function alerts(jwb,a){
    var id_siswa = "<?php echo $_SESSION[ids] ?>";
    var id_jadwal = "<?php echo $_GET[id_jadwal] ?>";
    var id_pertanyaan = a;
    $.ajax({
      type : "POST",
      url  : "insert_jwb_cbt.php",
      data : {id_siswa:id_siswa,id_jadwal:id_jadwal,id_pertanyaan:id_pertanyaan,jwb:jwb},
...
function showTab(n) {
  ...
  if (n == (x.length - 1)) {
    ...
    $("#finishBtn").hide()
    $("#nextBtn").show()
    document.getElementById("nextBtn").innerHTML = "Selanjutnya <i class='fa fa-angle-right'></i>";
  }
  ...
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <!-- <h5 class="modal-title" id="exampleModalLabel">Konfirmasi Selesai Ujian</h5> -->
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body text-center">
        Anda Yakin Akan Menyelesaikan Session CBT?
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Tidak</button>
        <button type="button" class="btn btn-primary" onclick="finished()">Ya</button>
      </div>
    </div>
  </div>
</div>