                  ...
                  <?php
                    ...
                    echo "<tr><td>$no</td>
                        ...
                        echo "<td style='width:70px !important'><center>
                                <a class='btn btn-success btn-xs' title='Tampil List Absensi' href='index.php?view=absensiswa&act=tampilabsen&id=$r[kode_kelas]&kd=$r[kode_pelajaran]&jdwl=$r[kodejdwl]&ie=$r[id_kls_pararel]'><span class='glyphicon glyphicon-th'></span> Tampilkan</a>
                              </center></td>";
                              }
                            echo "</tr>";
                      $no++;
                  ...
                  <table class='table table-condensed table-bordered table-striped'>
                    ...
                    echo "<tr bgcolor=$warna>
                            ...
                            <td><select style='width:100px;' name='a[$no]' class='form-control'>";
                                  $kehadiran = mysql_query("SELECT * FROM rb_kehadiran ORDER BY kode_kehadiran ASC");
                ...
                if (isset($_POST[simpann])){
                    ...
                    for ($i=1; $i <= $jml_data; $i++){
                      ...
                      if ($total >= 1){
                        ...
                        $cs = mysql_fetch_array(mysql_query("SELECT * FROM rb_siswa a JOIN rb_kelas b ON a.kode_kelas=b.kode_kelas a JOIN rb_kls_pararel c.id_kls_pararel=c.id_kls_pararel where a.nisn='".$nisn[$i]."'"));
                        if ($a[$i] != 'H'){
                        ...
                        $cs = mysql_fetch_array(mysql_query("SELECT * FROM rb_siswa a JOIN rb_kelas b ON a.kode_kelas=b.kode_kelas a JOIN rb_kls_pararel ON m.id_kls_pararel=m.id_kls_pararel where a.nisn='".$nisn[$i]."'"));
                        if ($a[$i] != 'H'){
                          if ($a[$i] == 'A'){ $statush = 'Alpa'; }elseif ($a[$i] == 'S'){ $statush = 'Sakit'; }elseif ($a[$i] == 'I'){ $statush = 'Izin'; }
                          $isi_pesan = "Diberitahukan kepada Yth Bpk/Ibk, Bahwa anak anda $cs[nama], $cs[nama_kelas] absensi Hari ini Tanggal $g-$f-$e : $statush";
                          if ($cs[no_telpon_ayah] != ''){
                ...
                <div class="box-body">
                  ...
                  <?php
                    ...
                    echo "<tr><td>$no</td>
                              ...
                              <td>$r[nama_ruangan]</td>
                              <td>$r[id_tahun_akademik]</td>
                              <td><a class='btn btn-success btn-xs' title='Tampil List Absensi' href='index.php?view=absensiswa&act=tampilabsen&id=$r[kode_kelas]&kd=$r[kode_pelajaran]&ie=$r[id_kls_pararel]'><span class='glyphicon glyphicon-th'></span> Tampil Absensi</a></td>
                          </tr>";
                      $no++;
                      }
                  ?>
