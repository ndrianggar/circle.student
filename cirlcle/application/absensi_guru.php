...
<?php 
    if (isset($_POST[simpan])){
      ...
      for ($i=1; $i <= $jml_data; $i++){
        $cek = mysql_query("SELECT * FROM rb_absensi_guru where kodejdwl='".$jdwl[$i]."' AND nip='".$nip[$i]."' AND tanggal='$tglabsen'");
        $total = mysql_num_rows($cek);
        if ($total >= 1){
           mysql_query("UPDATE rb_absensi_guru SET kode_kehadiran = '".$a[$i]."' where nip='".$nip[$i]."' AND kodejdwl='".$jdwl[$i]."' AND tanggal='$tglabsen'");
        }else{
             mysql_query("INSERT INTO rb_absensi_guru VALUES ('',
                                                              '".$jdwl[$i]."',
                                                              '".$nip[$i]."',
                                                              '".$a[$i]."',
                                                              '$tglabsen',
