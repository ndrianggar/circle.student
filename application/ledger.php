...
<div class="box-body" style="overflow-x:auto;">
  <table id="example" class="table table-bordered table-striped table-responsive">
    ...
    <tbody>
      <?php 
        $dataSiswa = mysql_query("SELECT c.*, a.nipd, a.nama, b.jenis_kelamin FROM rb_ledger c JOIN rb_siswa a ON c.nisn = a.nisn
                                              JOIN rb_jenis_kelamin b ON a.id_jenis_kelamin=b.id_jenis_kelamin 
                                                where c.kode_kelas='$_GET[kelas]' AND c.id_kls_pararel='$_GET[kelasp]' AND c.tahun = '$_GET[tahun]' GROUP BY c.nisn ORDER BY a.nama");$no=1;
        while ($s = mysql_fetch_array($dataSiswa)) {?>
          <tr>
            ...
            <?php 
              $data = mysql_query("SELECT a.namamatapelajaran, b.kodejdwl, b.kode_pelajaran FROM rb_mata_pelajaran a INNER JOIN rb_jadwal_pelajaran b ON a.kode_pelajaran = b.kode_pelajaran WHERE b.kode_kelas = '$_GET[kelas]' AND id_kls_pararel = '$_GET[kelasp]' AND id_tahun_akademik = '$_GET[tahun]' GROUP BY a.namamatapelajaran");
              while ($n= mysql_fetch_array($data)) {
                // $dataNilai =  mysql_fetch_array(mysql_query("SELECT FORMAT(a.nilai1,1) as nilai1, FORMAT(a.nilai2,1) as nilai2, FORMAT((a.nilai1*6+a.nilai2*4)/10,1) as raport, b.kode_pelajaran FROM rb_nilai_pengetahuan_semester a INNER JOIN rb_jadwal_pelajaran b ON a.kodejdwl = b.kodejdwl WHERE a.nisn = '$s[nisn]' AND b.kode_kelas = '$_GET[kelas]' AND b.id_kls_pararel= '$_GET[kelasp]' AND b.kode_pelajaran = '$n[kode_pelajaran]' AND a.tahun = '$_GET[tahun]'"));
                $dataNilai =  mysql_fetch_array(mysql_query("SELECT ub, uas, pengetahuan FROM rb_ledger WHERE nisn = '$s[nisn]' AND kode_kelas = '$_GET[kelas]' AND id_kls_pararel = '$_GET[kelasp]' AND kode_pelajaran = '$n[kode_pelajaran]' AND tahun = '$_GET[tahun]'"));
              echo "<td  style='background-color:#c1c1c1;'><b>".$dataNilai[ub]."</b></td><td style='background-color:#c1c1c1;'><b>".$dataNilai[uas]."</b></td><td style='background-color:#42f49e;'><b>".$dataNilai[pengetahuan]."</b></td>";
              }
