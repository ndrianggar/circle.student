...
if ($_GET[act]==''){ 
    if (isset($_POST[simpan])){
            if ($_POST[status]=='Update'){
              ...
              mysql_query("INSERT INTO rb_nilai_extrakulikuler VALUES('','$_GET[tahun]','$_POST[nisn]','$_GET[kelas]','$_GET[kelasp]','$_POST[a]','$_POST[b]','$_POST[c]','$_SESSION[id]','".date('Y-m-d H:i:s')."')");
            }
        echo "<script>document.location='index.php?view=extrakulikuler&tahun=$_GET[tahun]&kelas=$_GET[kelas]&kelasp=$_GET[kelasp]#$_POST[nisn]';</script>";
    }
                ...
                <div class="box-header">
                  ...
                  <form style='margin-right:5px; margin-top:0px' class='pull-right' action='' method='GET'>
					...
					<select name='kelasp' style='padding:4px'>
                        <?php 
                            echo "<option value=''>- Filter Kelas Paralel -</option>";
                            if ($_SESSION[unit]=="SMP") {
                ...
                <?php 
                  ...
                  if ($_GET[kelas] != '' AND $_GET[tahun] != ''){
                    ...
                    $tampil = mysql_query("SELECT * FROM rb_siswa a LEFT JOIN rb_kelas b ON a.kode_kelas=b.kode_kelas 
                                              LEFT JOIN rb_jenis_kelamin c ON a.id_jenis_kelamin=c.id_jenis_kelamin 
                                                LEFT JOIN rb_jurusan d ON b.kode_jurusan=d.kode_jurusan 
                                                  where a.kode_kelas='$_GET[kelas]' AND a.kode_kelas='$_GET[kelas]' ORDER BY a.id_siswa");
                  }
                    $no = 1;
                  ...
                  if ($_GET[nisn]==$r[nisn]){   
                    echo "<form action='index.php?view=extrakulikuler&tahun=$_GET[tahun]&kelas=$_GET[kelas]&kelasp=$_GET[kelasp]' method='POST'>
                            <tr><td>$no</td>
                              <td>$r[nisn]</td>
                    ...
                    echo "<form action='index.php?view=extrakulikuler&tahun=$_GET[tahun]&kelas=$_GET[kelas]&kelasp=$_GET[kelasp]' method='POST'>
                            ...
                            $pe = mysql_query("SELECT * FROM rb_nilai_extrakulikuler where id_tahun_akademik='$_GET[tahun]' AND nisn='$r[nisn]' AND kode_kelas='$_GET[kelas]' AND id_kls_pararel='$_GET[kelasp]'");
                            while ($n = mysql_fetch_array($pe)){
                                echo "<tr>
                                        ...
                                        <td>$n[deskripsi]</td>
                                        <td align=center><a href='index.php?view=extrakulikuler&tahun=".$_GET[tahun]."&kelas=".$_GET[kelas]."&kelasp=".$_GET[kelasp]."&edit=".$n[id_nilai_extrakulikuler]."&nisn=".$r[nisn]."#$r[nisn]' class='btn btn-xs btn-success'><span class='glyphicon glyphicon-edit'></span></a>
                                                        <a href='index.php?view=extrakulikuler&tahun=".$_GET[tahun]."&kelas=".$_GET[kelas]."&kelasp=".$_GET[kelasp]."&delete=".$n[id_nilai_extrakulikuler]."&nisn=".$r[nisn]."' class='btn btn-xs btn-danger' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a></td>
                                      </tr>";
                            }
                      $no++;
                      }
