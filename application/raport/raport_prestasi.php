...
if ($_GET[act]==''){ 
    if (isset($_POST[simpan])){
            if ($_POST[status]=='Update'){
              mysql_query("UPDATE rb_nilai_prestasi SET jenis_kegiatan='$_POST[a]', keterangan='$_POST[b]' where id_nilai_prestasi='$_POST[id]'");
            }else{
              mysql_query("INSERT INTO rb_nilai_prestasi VALUES('','$_GET[tahun]','$_POST[nisn]','$_GET[kelas]','$_GET[kelasp]','$_POST[a]','$_POST[b]','$_SESSION[id]','".date('Y-m-d H:i:s')."')");
            }
        echo "<script>document.location='index.php?view=prestasi&tahun=$_GET[tahun]&kelas=$_GET[kelas]&kelasp=$_GET[kelasp]#$_POST[nisn]';</script>";
    }
            ...
            <div class="col-xs-12">  
              <div class="box">
                ...
                <?php 
                  echo "<table id='example' class='table table-bordered table-striped'>
                   ...
                   if ($_GET[kelas] != '' AND $_GET[tahun] != ''){
                  ...
                  }elseif ($_GET[kelasp] != '' AND $_GET[tahun] != ''){
                    $tampil = mysql_query("SELECT * FROM rb_siswa a LEFT JOIN rb_kelas b ON a.kode_kelas=b.kode_kelas 
											LEFT JOIN rb_kls_pararel m ON a.id_kls_pararel=m.id_kls_pararel
                                              LEFT JOIN rb_jenis_kelamin c ON a.id_jenis_kelamin=c.id_jenis_kelamin 
                                                LEFT JOIN rb_jurusan d ON b.kode_jurusan=d.kode_jurusan 
                    ...
                    while($r=mysql_fetch_array($tampil)){
                      if (isset($_GET[edit])){
                          $e = mysql_fetch_array(mysql_query("SELECT * FROM rb_nilai_prestasi where id_nilai_prestasi='$_GET[edit]'"));
                          $name = 'Update';
                      }else{
                          $name = 'Simpan';
                      }
                  ...
                  if ($_GET[nisn]==$r[nisn]){   
                    echo "<form action='index.php?view=prestasi&tahun=$_GET[tahun]&kelas=$_GET[kelas]&kelasp=$_GET[kelasp]' method='POST'>
                            <tr><td>$no</td>
                              <td>$r[nisn]</td>
                    ...
                    echo "<form action='index.php?view=prestasi&tahun=$_GET[tahun]&kelas=$_GET[kelas]&kelasp=$_GET[kelasp]' method='POST'>
                            ...
                            $pe = mysql_query("SELECT * FROM rb_nilai_prestasi where id_tahun_akademik='$_GET[tahun]' AND nisn='$r[nisn]' AND kode_kelas='$_GET[kelas]' AND id_kls_pararel='$_GET[kelasp]'");
                            while ($n = mysql_fetch_array($pe)){
                                echo "<tr>
                                        ...
                                        <td>$n[keterangan]</td>
                                        <td width='90px' align=center><a href='index.php?view=prestasi&tahun=".$_GET[tahun]."&kelas=".$_GET[kelas]."&kelasp=".$_GET[kelasp]."&edit=".$n[id_nilai_prestasi]."&nisn=".$r[nisn]."#$r[nisn]' class='btn btn-xs btn-success'><span class='glyphicon glyphicon-edit'></span></a>
                                                        <a href='index.php?view=prestasi&tahun=".$_GET[tahun]."&kelas=".$_GET[kelas]."&kelasp=".$_GET[kelasp]."&delete=".$n[id_nilai_prestasi]."&nisn=".$r[nisn]."' class='btn btn-xs btn-danger' onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\"><span class='glyphicon glyphicon-remove'></span></a></td>
                                      </tr>";
                            }
                      $no++;
                      }
