<?php if ($_GET[act]==''){ 
  ...
  $m = mysql_fetch_array(mysql_query("SELECT * FROM rb_kls_pararel where id_kls_pararel='$_GET[ie]'"));
            ...
            <div class="col-xs-12">  
              <div class="box">
                <div class="box-header">
                  ...
                  <form style='margin-right:5px; margin-top:0px' class='pull-right' action='' method='GET'>
                    ...
                    <select name='id' style='padding:4px'>
					 ...
					 <select name='ie' style='padding:4px'>
                        <?php 
                ...
                <div class="box-body">
                    ...
                    <tbody>
                  <?php //ditambahkan where AND a.id_kls_pararel='$_GET[ie]'
                    $tampil = mysql_query("SELECT * FROM rb_siswa a
                                              JOIN rb_jenis_kelamin b ON a.id_jenis_kelamin=b.id_jenis_kelamin 
                                                where a.kode_kelas='$_GET[id]' AND a.id_kls_pararel='$_GET[ie]' ORDER BY a.id_siswa");
                    $no = 1;
                    ...
                    $rank = mysql_fetch_array(mysql_query("SELECT * FROM rb_peringkat where nisn='$r[nisn]' AND kode_kelas='$r[kode_kelas]' AND id_kls_pararel='$r[id_kls_pararel]' AND id_tahun_akademik='$_GET[tahun]'"));
                    
                    $cek = mysql_num_rows(mysql_query("SELECT * FROM rb_peringkat where nisn='$r[nisn]' AND kode_kelas='$r[kode_kelas]' AND id_kls_pararel='$r[id_kls_pararel]' AND id_tahun_akademik='$_GET[tahun]'"));
                    if ($cek > 0){
                      mysql_query("UPDATE rb_peringkat SET nilai_total='$nilai_total' where nisn='$r[nisn]' AND kode_kelas='$r[kode_kelas]' AND id_kls_pararel='$r[id_kls_pararel]' AND id_tahun_akademik='$_GET[tahun]'");
                    }else{
                      mysql_query("INSERT INTO rb_peringkat VALUES('','$r[nisn]','$r[kode_kelas]','$r[id_kls_pararel]','$_GET[tahun]','$nilai_total','')");
                    }
                    echo "<tr><td width=40px>$no</td>
                              ...
                              <td width='420px'><center>
                                ...
                                <a target='_BLANK' class='btn btn-success btn-xs' href='print_raport/print-hal4.php?id=$r[nisn]&kelas=$r[kode_kelas]&tahun=$_GET[tahun]&ie=$_GET[ie]'><span class='glyphicon glyphicon-print'></span> Hal 4</a>
                      ...
                      $rank = mysql_query("SELECT * FROM rb_peringkat WHERE  kode_kelas='$_GET[id]' AND id_kls_pararel='$_GET[ie]' ORDER BY nilai_total DESC");
                      $ii = 1;
                      
                      while ($ra = mysql_fetch_array($rank)){
                        mysql_query("UPDATE rb_peringkat SET rank='$ii' where nisn='$ra[nisn]' AND kode_kelas='$ra[kode_kelas]' AND id_kls_pararel='$ra[id_kls_pararel]' AND id_tahun_akademik='$_GET[tahun]'");
                        $ii++;
                      }
                  ?>
                    </tbody>
