                  ...
                  <?php
                   if ($_SESSION[level]!="guru") {
                      ...
                      $tampil = mysql_query("SELECT a.*, e.nama_kelas,m.nm_pararel, b.namamatapelajaran, b.kode_pelajaran, b.kode_kurikulum, c.nama_guru, d.nama_ruangan FROM rb_jadwal_pelajaran a 
                        ...
                        where a.id_kls_pararel='$_GET[kelasp]' 
                        AND a.kode_kelas='$_GET[kelas]'
                        AND a.id_tahun_akademik='$_GET[tahun]' 
                        AND b.kode_kurikulum='$kurikulum[kode_kurikulum]' ORDER BY a.hari DESC");
                    }elseif (isset($_GET[kelasp]) AND isset($_GET[tahun])){
                      $tampil = mysql_query("SELECT a.*, e.nama_kelas,m.nm_pararel, b.namamatapelajaran, b.kode_pelajaran, b.kode_kurikulum, c.nama_guru, d.nama_ruangan FROM rb_jadwal_pelajaran a 
                        JOIN rb_mata_pelajaran b ON a.kode_pelajaran=b.kode_pelajaran
                        JOIN rb_guru c ON a.nip=c.nip 
                        JOIN rb_ruangan d ON a.kode_ruangan=d.kode_ruangan
                     ...
                     if (isset($_GET[kelas]) AND isset($_GET[tahun])){
                      $tampil = mysql_query("SELECT a.*, e.nama_kelas,m.nm_pararel, b.namamatapelajaran, b.kode_pelajaran, b.kode_kurikulum, c.nama_guru, d.nama_ruangan FROM rb_jadwal_pelajaran a 
                        ...
                        AND a.nip = '$_SESSION[id]'  ORDER BY a.hari DESC");
                    }elseif (isset($_GET[kelasp]) AND isset($_GET[tahun])){
                      $tampil = mysql_query("SELECT a.*, e.nama_kelas,m.nm_pararel, b.namamatapelajaran, b.kode_pelajaran, b.kode_kurikulum, c.nama_guru, d.nama_ruangan FROM rb_jadwal_pelajaran a 
                        JOIN rb_mata_pelajaran b ON a.kode_pelajaran=b.kode_pelajaran
                        JOIN rb_guru c ON a.nip=c.nip 
                              ...
                              if (isset($_GET[tahun]) AND isset($_GET[kelas])){
                                if($_SESSION[level]!='kepala'){
                                  echo "<td style='width:80px !important'><center>
                                          <a class='btn btn-primary btn-xs' title='Lihat Nilai Keterampilan Siswa' href='index.php?view=ledgermapel&act=listsiswasikap&jdwl=$r[kodejdwl]&kd=$r[kode_pelajaran]&id=$r[kode_kelas]&ie=$r[id_kls_pararel]&tahun=$_GET[tahun]'><span class='glyphicon glyphicon-search'></span> Lihat</a>
                                        </center></td>";
                                }
                              }
                            echo "</tr>";
...
elseif($_GET[act]=='listsiswasikap'){
    if (isset($_POST[simpanpenilaian])){
      $juml = mysql_num_rows(mysql_query("SELECT * FROM rb_siswa where kode_kelas='$_GET[id]' AND id_kls_pararel='$_GET[ie]'"));
      for ($ia=1; $ia<=$juml; $ia++){
        $a   = $_POST['a'.$ia];
        $b   = $_POST['b'.$ia];
        $c   = $_POST['c'.$ia];
        ...
        if ($cek >= '1'){
          ...
          mysql_query("INSERT INTO rb_ledger VALUES('','$_GET[jdwl]','$_GET[kdpelajaran]','$nisn','$_GET[id]','$a','$b','$c','','','','".date('Y-m-d H:i:s')."','$_GET[tahun]')");
        }
      }
    }
    ...
    if (isset($_POST[simpandeskripsi])){
        ...
        for ($ia=1; $ia<=$juml; $ia++){
          ...
          $c   = $_POST['c'.$ia];
          $nisn   = $_POST['nisn'.$ia];
            $cek = mysql_num_rows(mysql_query("SELECT * FROM rb_ledger where kodejdwl='$_GET[jdwl]' AND nisn='$nisn' AND kode_kelas='$_GET[id]' AND id_kls_pararel='$_GET[ie]'"));
            if ($cek >= '1'){
              mysql_query("UPDATE rb_ledger SET desk_pengetahuan='$a', desk_keterampilan='$b', desk_sikap='$c' where kodejdwl='$_GET[jdwl]' AND nisn='$nisn' AND kode_kelas='$_GET[id]' AND id_kls_pararel='$_GET[ie]'");
            }else{
              mysql_query("INSERT INTO rb_ledger VALUES('','$_GET[jdwl]',$_GET[kdpelajaran],'$nisn','$_GET[id]','','','','$a','$b','$c','".date('Y-m-d H:i:s')."')");
            }
        }
        echo "<script>document.location='index.php?view=ledgermapel&act=listsiswasikap&jdwl=$_GET[jdwl]&kd=$_GET[kd]&id=$_GET[id]&ie=$_GET[ie]&tahun=$_GET[tahun]';</script>";
    }
        ...
        <div class='box-body'>
              ...
              <table class='table table-condensed table-hover'>
                  <tbody>
                    ...
                    <tr><th width='120px' scope='row'>Nama Kelas Pararel</th> <td>$n[nm_pararel]</td></tr>
            ...
            <div id='myTabContent' class='tab-content'>";
              ...
              // <li role='presentation' class=''><a href='#sosial' role='tab' id='sosial-tab' data-toggle='tab' aria-controls='sosial' aria-expanded='false'>Deskripsi</a></li>
                 //     <li role='presentation' class=''><a href='#hadir' role='tab' id='hadir-tab' data-toggle='tab' aria-controls='hadir' aria-expanded='false'>Kehadiran</a></li>
                      ...
                      echo "<div class='col-md-12'>
                            <form action='index.php?view=ledgermapel&act=listsiswasikap&jdwl=$_GET[jdwl]&kd=$_GET[kd]&id=$_GET[id]&tahun=$_GET[tahun]&ie=$_GET[ie]&kdpelajaran=$m[kode_pelajaran]' method='POST'>
                            ...
                            <table class='table table-bordered table-striped'>
                              ...
                              $rapnk = mysql_fetch_array(mysql_query("SELECT SUM((nilai1+nilai2+nilai3+nilai4+nilai5+nilai6)/6)/$contKd[count]  as total FROM rb_nilai_keterampilan where kodejdwl='$_GET[jdwl]' AND nisn='$r[nisn]'"));
                                  ...
                                  echo "<tr>
                                        ...
                                        <td align=center><input style='text-align:center' class='form-control' type='number' name='a".$no."' value='".number_format($total)."' readonly='on'></td>
                                        <td align=center><input style='text-align:center' class='form-control' type='number' name='b".$no."' value='".number_format($rapnk[total])."' readonly='on'></td>
                ...
                echo "<div role='tabpanel' class='tab-pane fade' id='sosial' aria-labelledby='sosial-tab'>
                      <div class='col-md-12'>
                            <form action='index.php?view=ledgermapel&act=listsiswasikap&jdwl=$_GET[jdwl]&kd=$_GET[kd]&id=$_GET[id]&tahun=$_GET[tahun]&kdpelajaran=$m[kode_pelajaran]' method='POST'>
                            ...
                            <table class='table table-bordered table-striped'>
                              ...
                              <tbody>";
                              $no = 1;
                              $tampil = mysql_query("SELECT * FROM rb_siswa where kode_kelas='$_GET[id]' AND id_kls_pararel='$_GET[ie]' ORDER BY id_siswa");
                              while($r=mysql_fetch_array($tampil)){
                                $des = mysql_fetch_array(mysql_query("SELECT * FROM rb_ledger where kodejdwl='$_GET[jdwl]' AND nisn='$r[nisn]' AND kode_kelas='$_GET[id]'"));
                                  echo "<tr>
                                        <td>$no</td>
